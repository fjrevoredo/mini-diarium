...
10. **`aria-expanded` does not mean the mobile sidebar has finished opening.** The overlay sidebar's `transition-transform` runs for 300 ms, while the hamburger's `aria-expanded` changes immediately. WebDriver can therefore report a calendar button as clickable while WebKitGTK still hit-tests the moving overlay and loses the click. Before interacting with calendar controls after opening the sidebar, wait for the sidebar's transformed position (`#sidebar` `getLocation('x') >= 0`) rather than waiting on `aria-expanded` alone.
