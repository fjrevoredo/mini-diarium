...
### Internal
- **Fixed the `msstore publish` invocation in `msstore-publish.yml`** (TODO-0078): the Microsoft Store update step passed the Store product id as the positional `pathOrUrl` argument (which the `msstore` CLI resolves as a project directory/PWA URL) and the `.msix` via `--inputFile`. Corrected to Microsoft's documented MSIX package-update form — `msstore publish <path-to-.msix> -id <productId>` — so the never-run step does not fail on a malformed command on its first dry run. CI-only; the step still only runs when the five `PARTNER_CENTER_*`/`MSSTORE_PRODUCT_ID` secrets are configured.
- **Fixed the MSIX pack step in `msstore-publish.yml`** (TODO-0078): the "Assemble MSIX payload and pack" step ran `pwsh ./scripts/build-msix.ps1 …` inside a step that is already `shell: pwsh`, spawning a nested pwsh that failed on the CI runner with "`./scripts/build-msix.ps1` is not recognized as the name of a script file". Now invokes the script directly via the call operator against an absolute `${{ github.workspace }}` path, removing the redundant nested process and any working-directory ambiguity. Surfaced by the first-ever dry run of the workflow. CI-only.
