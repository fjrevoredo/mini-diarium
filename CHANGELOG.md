...
- **Docker Runtime (Linux container, browser-rendered desktop)**: Mini Diarium can now run as a supported containerized desktop appliance. The runtime publishes an authenticated GUI only on `127.0.0.1`, persists journals and app state in a named `/data` volume, limits host file exchange to `/exchange`, and removes the container default route before any plaintext-bearing process starts. The repository now includes a source-controlled Dockerfile, Compose deployment, bootstrap/health scripts, CI smoke validation, and GHCR release publishing.
