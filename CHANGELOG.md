...
### Internal
- **Stable `mini-diarium-core` façade API — open-core M2** (TODO-0077): the core crate now exposes **one deliberate public surface** (open/unlock, entry CRUD, search, import/export, auth-slot management) instead of letting the app reach into internal module paths. The `db::queries` / `db::schema` sub-modules and the `auth`/`export`/`import`/`plugin` sub-modules are now `pub(crate)`; the curated API is re-exported at each module root, so call sites lose the `::queries::` / `::schema::` / `::password::` middle segment (e.g. `crate::db::queries::insert_entry` → `crate::db::insert_entry`). `DatabaseConnection::conn()` / `key()` are sealed to `pub(crate)` — no raw `rusqlite` handle and no master key ever escape the crate; new composed operations (`auth::add_password_slot` / `add_keypair_slot`) encapsulate the master-key wrap, `insert_entry` now returns the new row id, dedicated query functions replace every bespoke-SQL call site, and `db_settings` gained `&DatabaseConnection` wrappers. Search moved into a new `mini_diarium_core::search` module (the Tauri `search_entries` command is now a thin wrapper); serde field names are unchanged, so the frontend/IPC contract is untouched. The stability contract is documented in the new `crates/mini-diarium-core/API.md`; an automated `cargo-public-api` guard is a deferred follow-up. Behavior-preserving backend + docs change — all backend tests pass (`cargo test --workspace`), clippy is clean (`--workspace --all-targets -D warnings`), and no user-visible change.
- **Extracted the `mini-diarium-core` crate — open-core M1 workspace split** (TODO-0076): the Rust backend is now a repo-root virtual Cargo workspace with two members. The Tauri-free business layer (`crypto/`, `auth/`, `db/`, `import/`, `export/`, `plugin/`, `backup.rs`, `config.rs`, and the shared domain types) moved verbatim into a new `crates/mini-diarium-core/` library crate that has **zero `tauri` references**, so it can later back a separate product and compile to WASM (see `docs/OPEN_CORE_STRATEGY.md`). The app crate stays at `src-tauri/` (Tauri commands + OS shell) and depends on the new crate via a path dependency; its `lib.rs` re-exports the core modules so every existing `crate::db::…`-style path in `commands/*` and the benches resolves unchanged. This is a behavior-preserving code move — no logic changes; all backend tests pass identically. The shared `[profile.release]` and `Cargo.lock` now live at the workspace root, and the build target dir moved from `src-tauri/target/` to repo-root `target/`. The core crate carries a fixed `version = "0.1.0"` intentionally decoupled from the app version. The split forced a tooling migration: CI/release/benchmark/flathub workflows, Codecov + `check-diff-coverage.mjs`, `pre-commit` (hook + script), `nix/package.nix`, Dependabot, the no-network static check (now scans both `Cargo.toml` manifests), `bump-version.sh`/`.ps1`, `wdio.conf.ts`, MSIX/Flatpak paths, and the `CLAUDE.md`/best-practices/runbook docs all updated for the two-crate layout and the mandatory `cargo test --workspace`. No user-visible change.
