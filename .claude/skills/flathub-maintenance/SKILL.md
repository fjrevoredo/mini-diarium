...
### 2a. Stale or incomplete `package-lock.json` — lockfile integrity gap
...
- **Divergence:** `bun.lock` is current but `package-lock.json` was never regenerated with real npm after a `package.json` change. Run `node flatpak/check-node-sources.mjs` and compare.
- **Incomplete entries:** `package-lock.json` was regenerated with `npm install --package-lock-only --legacy-peer-deps`, but some entries lack `resolved`/`integrity` (see Failure 2 "Hidden root cause" above). Search: `Select-String -Path package-lock.json -Pattern '"node_modules/zip-stream"' -Context 0,5` and check for `resolved` field.

**Fix — when regenerating the lockfile:**
```bash
npm install --package-lock-only --ignore-scripts --legacy-peer-deps
```
npm may say "up to date" but still modify the file if packages were added since the last generation.

**Fix — when lockfile entries are missing `resolved`/`integrity`:** Delete `node_modules/` and run a full install:
```bash
npm install --legacy-peer-deps
```
Then run `bun install` to sync `bun.lock`. Verify with `check-node-sources.mjs`.
