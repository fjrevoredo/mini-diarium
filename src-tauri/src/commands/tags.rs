...
pub fn get_tags_for_entry(entry_id: i64, state: State<DiaryState>) -> Result<Vec<Tag>, String> {
    with_unlocked_db(&state, |db| db::get_tags_for_entry(db, entry_id))
}
...
pub fn get_entry_dates_by_tag(
    ...
    with_unlocked_db(&state, |db| db::get_entry_dates_by_tag(db, tag_id))
}
...
mod tests {
    ...
    fn state_with_db(db: crate::db::DatabaseConnection, name: &str) -> DiaryState {
    ...
    fn insert_entry_return_id(db: &crate::db::DatabaseConnection, date: &str) -> i64 {
        ...
        db::insert_entry(db, &entry).unwrap()
    }
    ...
    fn test_add_tag_to_locked_entry_is_rejected() {
        ...
        let tag = db::create_tag(&db, "work").unwrap();
        ...
        let tags = db::get_tags_for_entry(db_guard.as_ref().unwrap(), entry_id).unwrap();
    ...
    fn test_remove_tag_from_locked_entry_is_rejected() {
        ...
        let tag = db::create_tag(&db, "personal").unwrap();
        // Associate the tag while still unlocked, then lock.
        db::add_tag_to_entry(&db, entry_id, tag.id).unwrap();
        db::set_entry_locked(&db, entry_id, true).unwrap();
    ...
    fn test_add_tag_to_unlocked_entry_succeeds() {
        ...
        let tag = db::create_tag(&db, "idea").unwrap();
