...
- **Offline-only application process** — the Mini Diarium application process makes no network requests. No diary content is transmitted to any server at any point. The About screen and onboarding tour surface links to GitHub and documentation; clicking these hands the URL to the OS, which opens your system browser. Mini Diarium itself makes no network call — the browser's connection is independent and outside the app's scope. In the Docker runtime, the container appliance accepts a localhost-only GUI connection from your browser on `127.0.0.1`, but the plaintext-bearing processes inside the container start only after the bootstrap removes the default route and drops runtime capabilities.
- **Defense-in-depth WebView isolation** — multiple layers prevent the embedded WebView from making outbound network requests: strict CSP (`connect-src 'self' ipc: http://ipc.localhost`; `worker-src`, `child-src`, `frame-src`, `object-src`, `form-action`, `manifest-src` all `'none'`); a document-start init script that nulls `RTCPeerConnection`, `WebTransport`, `Worker`, `SharedWorker`, `navigator.serviceWorker`, `navigator.sendBeacon`, `navigator.connection`, and `window.open` in all frames; `on_navigation` that blocks any non-app URL; and `on_new_window(Deny)` that blocks popup creation on all platforms. `fetch`/`XMLHttpRequest`/`WebSocket`/`EventSource` remain available for Tauri IPC and local app/runtime communication, with outbound access constrained by CSP and platform handlers. Windows builds additionally register a `WebResourceRequested` COM handler; macOS builds register a `WKContentRuleList` content-blocking rule.
...
- **macOS `.app` distribution is not App Sandboxed** — the macOS DMG/`.app` build provides WebView-layer network isolation (CSP, init script, `WKContentRuleList`) but not OS-level process network isolation. The Rust process has normal network access in principle; this is not OS-enforced. The process makes no network calls by design, but users who require a hard OS-enforced guarantee should wait for a future sandboxed release. Sandboxing is deferred because it requires complex entitlement work for arbitrary journal file paths.
- **Linux AppImage/deb builds have no OS-level network isolation** — these builds have normal process network access. The WebView-layer defenses (CSP, init script, `on_navigation`) still apply, but there is no kernel namespace isolation. Users who require hard isolation should prefer the Flatpak package, which runs without `--share=network`, or the Docker runtime, which removes the default route before the GUI stack starts.
- **Docker runtime adds a localhost GUI transport** — the Docker runtime is not a network API, but it does add an authenticated browser-rendered desktop on `127.0.0.1`. The GUI password reduces casual same-host access to an unlocked session; it does not defend against malicious local software, screen capture malware, or a host user who can read the secret file. Publishing the Docker runtime beyond localhost is unsupported and unsafe.
