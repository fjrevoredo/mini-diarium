...
- Multiple journals are tracked in `{app_data_dir}/config.json` via `JournalConfig` entries. Each journal maps to a specific database file — a `path` (folder) plus a `db_filename` (defaults to `diary.db` when absent) — so several journals can share one folder as long as their filenames differ; only an identical `path` + `db_filename` pair is refused as a duplicate. `DiaryState` holds a single connection; switching journals updates `db_path`/`backups_dir` and auto-locks. Legacy single-diary configs are auto-migrated on first `load_journals()` call.
...
Use `$runbooks pre-release` in Codex or `/runbooks pre-release` in Claude Code, then follow [docs/releasing/RELEASING.md](docs/releasing/RELEASING.md) for the full process. From this shell, route project commands through `cmd.exe /c ...`. Version bump script: `./bump-version.sh X.Y.Z`.

**Distribution channels** on each tagged release: GitHub Releases (installers) plus WinGet, Homebrew, Flathub, and the **Microsoft Store** (MSIX, Store-signed). The Store pipeline lives in [`msix/`](msix/README.md) + [`scripts/build-msix.ps1`](scripts/build-msix.ps1) + [`.github/workflows/msstore-publish.yml`](.github/workflows/msstore-publish.yml); the first Store submission is manual, updates are CI-dispatched (non-blocking) from `release.yml`. See [docs/releasing/RELEASING.md](docs/releasing/RELEASING.md) → "Microsoft Store (MSIX)". **Windows release artifacts are code-signed via SignPath** (Foundation OSS tier, test certificate) — see [docs/releasing/RELEASING.md](docs/releasing/RELEASING.md) → "Windows Code Signing (SignPath)" and [docs/releasing/SIGNPATH_FIRST_TIME_SETUP.md](docs/releasing/SIGNPATH_FIRST_TIME_SETUP.md) for the from-scratch dashboard walkthrough.
