...
- [x] **Supported-language list propagation** — prospective users have no way to see which languages the app ships in without downloading and launching it; surface the supported-language list in all public-facing distribution points and keep it automatically in sync so adding a new locale never requires manual edits across multiple files
