...
- [ ] **Basic embedded images in the editor** (#40) — support inserting and rendering inline images in diary entries, starting with the drag-and-drop flow that currently shows an OS-level “copy” affordance on macOS but does nothing in the editor; align PHILOSOPHY/docs wording and the UI capability so “basic embedded images” is either truly supported or not implied
- [ ] **Text highlight formatting** (#41) — add a highlight/marker formatting option in the editor to improve scanability; start with a single theme-safe color and place it in the advanced formatting toolbar if shipped, then verify how it should round-trip through stored HTML plus Markdown/JSON export paths
