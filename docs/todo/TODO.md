...
- [x] **TODO-0053: Wire search UI into the app shell** — `SearchBar` and `SearchResults` components exist and the backend `search_entries` command is implemented (PR #171); decide on a placement (sidebar, overlay, or command palette), mount the components behind `VITE_EXPERIMENTAL`, then promote both the Rust `#[cfg(feature = "experimental")]` gate in `lib.rs` and the frontend `<Show when={import.meta.env.VITE_EXPERIMENTAL}>` guard to production in the same PR once the UI is ready

- [x] **TODO-0059: Frontend tests for search deep-link and stale-response guard** — PR #171 added frontend behaviour that ships untested and dormant until the search UI is mounted (TODO-0053): the `selectedEntryId` deep-link consumed in `useEntryLifecycle.loadEntriesForDate` (target entry found → open it; not found → fall back to the day's newest), the same-day deep-link effect in `EditorPanel.tsx`, and the monotonic `searchSeq` latest-wins guard in `SearchBar.tsx`; add Vitest coverage for both deep-link branches and the stale-response guard so the path is tested before it is exercised end to end
...
- [x] **TODO-0057: Split `src/lib/tauri.ts` below the 500-line hard limit** — file is at ~510 lines, 10 lines over the hard limit for Tauri boundary files; extract the largest command groups (entries, auth, export) into namespaced sub-files re-exported from a barrel, or trim enough of the largest groups to bring the single file below the limit
- [x] **TODO-0060: Accent-insensitive search** — `search_entries` (PR #171) is case-insensitive but not accent-insensitive ("café" ≠ "cafe"); add Unicode accent folding (e.g. a `unicode-normalization` dependency, NFD normalization plus combining-mark stripping) to both `normalize_terms` and the text-folding path in `src-tauri/src/commands/search.rs`, keeping the `lower_with_map` byte-offset map valid so snippet boundaries stay char-boundary-safe; originally gated behind the `experimental` search feature, now part of production after TODO-0053 promoted search
