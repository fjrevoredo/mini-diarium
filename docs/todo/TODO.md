...
**Latest TODO ID: TODO-0065** — next new TODO should be TODO-0066
...
- [ ] **TODO-0061: Add header overflow menu with in-app Preferences access** — build a reusable Kobalte `DropdownMenu`-based overflow menu triggered by a new `⋮` icon in `Header.tsx`'s right icon cluster; wire "Preferences" as the first item, opening `PreferencesOverlay` via the existing `setIsPreferencesOpen` state setter (same call already made by the `menu-preferences` listener in `MainLayout.tsx`); style consistent with existing overlay animation classes; add `data-testid` entries to the canonical table in `src/CLAUDE.md`; see [TODO-0061-01](TODO_EXTRA.md#todo-0061-01-header-overflow-menu-foundation) for exact file/line references
