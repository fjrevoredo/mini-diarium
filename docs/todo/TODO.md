...
- [x] **TODO-0051: AppImage GLIBC compatibility with Debian 12 (bookworm)** — the Linux AppImage (v0.5.3) requires `GLIBC_2.38` / `GLIBC_2.39` at runtime, which is not available on Debian 12 (bookworm, ships ~2.36); users on stable LTS distros cannot launch the AppImage; investigate building the AppImage against an older glibc baseline (e.g. via a Debian 12 or Ubuntu 22.04 CI runner), or provide a static/Flatpak alternative for older distros (Closed: root cause was `.github/workflows/release.yml`'s Linux matrix entry building on `ubuntu-latest`, which resolves to Ubuntu 24.04 with glibc 2.39 — newer than Debian 12's 2.36. Changed to `os: ubuntu-22.04` (glibc 2.35), which is older than Debian 12's floor, so the resulting AppImage now runs there and on any distro with glibc ≥2.35. Confirmed via `actions/runner-images` that `ubuntu-22.04` is still a GA image — `ubuntu-26.04` is only in preview, and per GitHub's "2 GA images" policy 22.04's deprecation window hasn't started, so no imminent revisit is needed. Empirical verification (building on the new runner, checking `objdump` glibc symbol versions, launching under a Debian 12 container) was not performed in this session — no Linux/Docker environment was available. A `workflow_dispatch` trigger was added to `release.yml` (gated so `create-release`/`publish-release` only run on tag pushes) so this can be dry-run on demand without cutting a release; deferred to that dry-run or the next real release build. CHANGELOG.md `[0.6.1] - Unreleased` → `Fixed` entry added.)
...
- [x] **TODO-0016: `DiaryEntry` clone efficiency** — `DiaryEntry` in `db/queries.rs` derives `Clone` and can be heap-copied across import/export flows; pass references where possible to reduce allocations when processing thousands of entries; audit current command and export call sites (Closed: audit found no `.clone()`/`.cloned()`/`.to_vec()`/`.to_owned()` call sites on `DiaryEntry` or `Vec<DiaryEntry>` anywhere in `src-tauri/src/`; export/import/search/plugin pipelines already move `Vec<DiaryEntry>` ownership once and iterate by reference (`&[DiaryEntry]`, `&entry`) thereafter. `Clone` derive kept — unused but zero-cost and consistent with the struct's other derives.)
...
- [x] **TODO-0049: Replace `navigator.platform` with `navigator.userAgentData`** — `navigator.platform` is deprecated; replace with `(navigator.userAgentData?.platform ?? navigator.platform ?? '').toLowerCase()` and update the `isMac` check accordingly; one call site: `src/components/editor/extensions/LinkWithDialog.ts` (`getLinkOpenShortcutLabel`); no behavioral change expected — Tauri WebViews support both APIs
